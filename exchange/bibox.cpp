...
void   bibox::threadfunc_stream()
{
    ...
    h.onConnection([this](uWS::WebSocket<uWS::CLIENT> *ws, uWS::HttpRequest req) {
        ...
        case 8:
            std::cout << "bibox Client established a remote connection over non-SSL" << std::endl;
            //ws->close(1000);
            break;
        case 9:
            std::cout << "bibox Client established a remote connection over SSL" << std::endl;
            this->ws = ws;
    ...
    h.onDisconnection([](uWS::WebSocket<uWS::CLIENT> *ws, int code, char *message, size_t length) {
        std::cout << "bibox Client got disconnected with data: " << ws->getUserData() << ", code: " << code << ", message: <" << std::string(message, length) << ">" << std::endl;
    });
    ...
    //h.connect("wss://api.bibox.com:9999/websocket", (void *) 9);
    //h.connect("ws://127.0.0.1:6000", (void *) 10, {}, 60000);

    cout<<"bibox websocket connect......"<<endl;
    h.run();
    cout << "Falling through testConnections" <<endl;
}

void   bibox::start_stream()
{
    thread t(&bibox::threadfunc_stream,this);
    t.detach();

    unique_lock<mutex> lock(mu);
    cout<<"bibox wait the wss connect complete---"<<endl;
    cv.wait(lock, [this] {return wsconn_state;});
    cout<<"bibox wss connect is complete---"<<endl;
}

void   bibox::closestream()
{
    this->ws->terminate();
}

void   bibox::subscribe_depth(string symbol)
{
    ...
    unique_lock<mutex> lock(mu);
    cout<<"bibox wait the sub complete---"<<endl;
    cv.wait(lock, [this] {return sub_state;});
    cout<<"bibox sub is complete---"<<endl;
}

void   bibox::cancel_subscribe_depth(string symbol)
{
    this->sendmsg(symbol);
}

void   bibox::sendmsg(string msg)
{
    this->ws->send(msg.c_str());
}

void   bibox::parse_priceamount_to_map(string symbol,const Value &data)
{
