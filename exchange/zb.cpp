...
void   zb::threadfunc_stream()
{
    ...
    h.onConnection([this](uWS::WebSocket<uWS::CLIENT> *ws, uWS::HttpRequest req) {
        ...
        case 8:
            std::cout << "zb Client established a remote connection over non-SSL" << std::endl;
            //ws->close(1000);
            break;
        case 9:
            std::cout << "zb Client established a remote connection over SSL" << std::endl;
            this->ws = ws;
    ...
    h.onDisconnection([](uWS::WebSocket<uWS::CLIENT> *ws, int code, char *message, size_t length) {
        std::cout << "zb Client got disconnected with data: " << ws->getUserData() << ", code: " << code << ", message: <" << std::string(message, length) << ">" << std::endl;
    });
    ...
    cout<<"zb websocket connect......"<<endl;
    h.run();
    cout << "Falling through testConnections" <<endl;
}

void   zb::start_stream()
{
    thread t(&zb::threadfunc_stream,this);
    t.detach();

    unique_lock<mutex> lock(mu);
    cout<<"zb wait the wss connect complete---"<<endl;
    cv.wait(lock, [this] {return wsconn_state;});
    cout<<"zb wss connect is complete---"<<endl;
}
...
void   zb::subscribe_depth(string symbol)
{
    ...
    unique_lock<mutex> lock(mu);
    cout<<"zb wait the sub complete---"<<endl;
    cv.wait(lock, [this] {return sub_state;});
    cout<<"zb sub is complete---"<<endl;
}

void   zb::cancel_subscribe_depth(string symbol)
{
    this->sendmsg(symbol);
}

void   zb::sendmsg(string msg)
{
    this->ws->send(msg.c_str());
}

void   zb::parse_priceamount_to_map(string symbol,const Value &data)
{
