...
void   cryptofacilities::threadfunc_stream()
{
    ...
    h.onConnection([this](uWS::WebSocket<uWS::CLIENT> *ws, uWS::HttpRequest req) {
        ...
        case 8:
            std::cout << "cryptofacilities Client established a remote connection over non-SSL" << std::endl;
            //ws->close(1000);
            break;
        case 9:
            std::cout << "cryptofacilities Client established a remote connection over SSL" << std::endl;
            this->ws = ws;
    ...
    h.onDisconnection([](uWS::WebSocket<uWS::CLIENT> *ws, int code, char *message, size_t length) {
        std::cout << "cryptofacilities Client got disconnected with data: " << ws->getUserData() << ", code: " << code << ", message: <" << std::string(message, length) << ">" << std::endl;
    });
    ...
    //h.connect("wss://api.cryptofacilities.com:9999/websocket", (void *) 9);
    //h.connect("ws://127.0.0.1:6000", (void *) 10, {}, 60000);

    cout<<"cryptofacilities websocket connect......"<<endl;
    h.run();
    cout << "Falling through testConnections" <<endl;
}

void   cryptofacilities::start_stream()
{
    thread t(&cryptofacilities::threadfunc_stream,this);
    t.detach();

    unique_lock<mutex> lock(mu);
    cout<<"cryptofacilities wait the wss connect complete---"<<endl;
    cv.wait(lock, [this] {return wsconn_state;});
    cout<<"cryptofacilities wss connect is complete---"<<endl;
}

void   cryptofacilities::closestream()
{
    this->ws->terminate();
}

void   cryptofacilities::subscribe_depth(string symbol)
{
    ...
    unique_lock<mutex> lock(mu);
    cout<<"cryptofacilities wait the sub complete---"<<endl;
    cv.wait(lock, [this] {return sub_state;});
    cout<<"cryptofacilities sub is complete---"<<endl;
}

void   cryptofacilities::cancel_subscribe_depth(string symbol)
{
    this->sendmsg(symbol);
}

void   cryptofacilities::sendmsg(string msg)
{
    this->ws->send(msg.c_str());
}

void   cryptofacilities::parse_priceamount_to_map(string symbol,const Value &data)
{
