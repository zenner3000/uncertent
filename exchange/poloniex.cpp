...
/*h,out
  ws,out
*/
void   poloniex::start_stream(){

    thread t(&poloniex::threadfunc_stream,this);
    t.detach();

    unique_lock<mutex> lock(mu);
    cout<<"poloniex wait the wss connect complete---"<<endl;
    cv.wait(lock, [this] {return wsconn_state;});
    cout<<"poloniex wss connect is complete---"<<endl;
    //t.join();
    //return t.get_id();
}

void   poloniex::closestream(){
    //this->h->close();
    this->ws->terminate();
}
...
bidtable, out
asktable, out*/
void   poloniex::subscribe_depth(string symbol){
...
/*ws,in
subststr,in*/
void   poloniex::cancel_subscribe_depth(string subststr){
    this->sendmsg(subststr);
}


void   poloniex::sendmsg(string msg){
    this->ws->send(msg.c_str());
}


//string symbol, map<double,double>  &bidtable, map<double,double>  &asktable
void   poloniex::threadfunc_stream(){
    ...
    h.onDisconnection([](uWS::WebSocket<uWS::CLIENT> *ws, int code, char *message, size_t length) {
        std::cout << "poloniex Client got disconnected with data: " << ws->getUserData() << ", code: " << code << ", message: <" << std::string(message, length) << ">" << std::endl;
    });
...
//https://github.com/Tencent/rapidjson/blob/master/example/tutorial/tutorial.cpp
    ...
    cout<<"poloniex websocket connect......"<<endl;
    h.run();
    cout << "Falling through testConnections" <<endl;

}

void   poloniex::parse_priceamount_to_map(string symbol,const Value &data){
