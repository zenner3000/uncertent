...
void   coinbase::threadfunc_stream()
{
    ...
    h.onConnection([this](uWS::WebSocket<uWS::CLIENT> *ws, uWS::HttpRequest req) {
        ...
        case 8:
            std::cout << "coinbase Client established a remote connection over non-SSL" << std::endl;
            //ws->close(1000);
            break;
        case 9:
            std::cout << "coinbase Client established a remote connection over SSL" << std::endl;
            this->ws = ws;
    ...
    h.onDisconnection([](uWS::WebSocket<uWS::CLIENT> *ws, int code, char *message, size_t length) {
        std::cout << "coinbase Client got disconnected with data: " << ws->getUserData() << ", code: " << code << ", message: <" << std::string(message, length) << ">" << std::endl;
    });
    ...
    //h.connect("wss://api.coinbase.com:9999/websocket", (void *) 9);
    //h.connect("ws://127.0.0.1:6000", (void *) 10, {}, 60000);

    cout<<"coinbase websocket connect......"<<endl;
    h.run();
    cout << "Falling through testConnections" <<endl;
}

void   coinbase::start_stream()
{
    thread t(&coinbase::threadfunc_stream,this);
    t.detach();

    unique_lock<mutex> lock(mu);
    cout<<"coinbase wait the wss connect complete---"<<endl;
    cv.wait(lock, [this] {return wsconn_state;});
    cout<<"coinbase wss connect is complete---"<<endl;
}

void   coinbase::closestream()
{
    this->ws->terminate();
}

void   coinbase::subscribe_depth(string symbol)
{
    ...
    unique_lock<mutex> lock(mu);
    cout<<"coinbase wait the sub complete---"<<endl;
    cv.wait(lock, [this] {return sub_state;});
    cout<<"coinbase sub is complete---"<<endl;
}

void   coinbase::cancel_subscribe_depth(string symbol)
{
    this->sendmsg(symbol);
}

void   coinbase::sendmsg(string msg)
{
    this->ws->send(msg.c_str());
}

void   coinbase::parse_priceamount_to_map(string symbol,const Value &data)
{
